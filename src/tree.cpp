...
// non-const: returns mutable pointers
void tree::get_bots(npv& bv)
{
  if (l && r) {
    ...
    bv.push_back(this);    // OK: this is tree*
  }
}

// const: returns const pointers
void tree::get_bots(cnpv& bv) const
{
  if (l && r) {
    l->get_bots(bv);       // OK: calling const overload on children
    r->get_bots(bv);
  } else {
    bv.push_back(this);    // OK: this is const tree*
  }
}
