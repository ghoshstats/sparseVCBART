...
void fit_ensemble(std::vector<double> &fit, std::vector<tree> &t_vec, data_info &di){
  ...
  for(int i = 0; i < di.N; i++){
    ...
    if(di.z_cat != 0) zz_cat = di.z_cat + i * di.R_cat;
    fit[i] = 0.0;
    for(int m = 0; m < (int)t_vec.size(); m++) fit[i] += t_vec[m].evaluate(zz_cont, zz_cat); // involves a tree traversal
  }
}
...
void compute_p_theta_cs(int &j, arma::mat &P, arma::vec &Theta, std::map<int,int> &leaf_map, suff_stat &ss, double &rho, double &sigma, data_info &di, tree_prior_info &tree_pi)
{
  ...
  for(suff_stat_it l_it = ss.begin(); l_it != ss.end(); ++l_it){
    ...
    xr_sums.insert(std::pair<int, double>(l_it->first, 0.0)); // initialze total sum of x_ijt*r_it in each leaf
...
// ---------- ROBUST version of compute_lil (stable Cholesky + ridge) ----------

double compute_lil(arma::mat &P, arma::vec &Theta, tree_prior_info &tree_pi)
{
  const int L = Theta.size(); // number of leafs

  // Cholesky with small ridge for stability
  arma::mat K = P;
  arma::mat Lch;
  bool ok = arma::chol(Lch, K, "lower");
  if(!ok){
    K.diag() += 1e-8;
    ok = arma::chol(Lch, K, "lower");
  }
  if(!ok){
    K.diag() += 1e-6;
    ok = arma::chol(Lch, K, "lower");
  ...
  if(!ok){
    Rcpp::Rcout << "[[compute_lil]]: Cholesky failed even after ridge; matrix likely ill-conditioned." << std::endl;
    K.print("Posterior precision (with ridge) K:");
    Rcpp::stop("Invalid posterior precision matrix for this tree.");
  }

  // Pinv * Theta via triangular solves
  arma::vec y = arma::solve(arma::trimatl(Lch), Theta);
  double quad = arma::dot(y, y); // = Theta' * P^{-1} * Theta
  double logdet = 2.0 * arma::sum(arma::log(Lch.diag()));

  // -0.5 * log det P  - L * log(tau) + 0.5 * Theta' P^{-1} Theta - 0.5 * L * mu0^2/tau^2
  double lil = - static_cast<double>(L) * std::log(tree_pi.tau)
               - 0.5 * static_cast<double>(L) * (tree_pi.mu0 * tree_pi.mu0) / (tree_pi.tau * tree_pi.tau);
  lil += 0.5 * quad;
  lil -= 0.5 * logdet;
  return lil;
}

// ---------- ROBUST sampler for mu ~ N(P^{-1}Theta, P^{-1}) via Cholesky ----------
...
void draw_mu(tree &t, arma::mat &P, arma::vec &Theta, std::map<int,int> &leaf_map, RNG &gen)
{
  ...
  if(!ok){
    Rcpp::Rcout << "[draw_mu]: Cholesky failed even after ridge." << std::endl;
    Rcpp::stop("Cannot sample mu: posterior precision not SPD.");
  }

  // posterior mean m = P^{-1} Theta (two triangular solves)
  arma::vec y = arma::solve(arma::trimatl(Lch), Theta);
  arma::vec m = arma::solve(arma::trimatu(Lch.t()), y);

  // sample eps ~ N(0, P^{-1}) via L^{-T} z
  arma::vec z = arma::randn<arma::vec>(Theta.n_elem);
  arma::vec eps = arma::solve(arma::trimatu(Lch.t()), z);

  arma::vec mu = m + eps;

  // write back into the leaves using the provided index map
  tree::tree_p bn;
...
// independent-errors: same as compute_p_theta_ind but with scalar prior variance per leaf
                            ...
                            double prior_var_per_leaf)
{
  ...
  P.set_size(L,L);
  P.eye();
  P.diag() *= 1.0 / prior_var_per_leaf;

  Theta.set_size(L);
  Theta.ones();
  Theta *= tree_pi.mu0 / prior_var_per_leaf;
  ...
  for(suff_stat_it l_it = ss.begin(); l_it != ss.end(); ++l_it){
    ...
    for(int_it it = l_it->second.begin(); it != l_it->second.end(); ++it){
      i = *it;
      P(l,l) += (1.0/(sigma*sigma)) * pow(di.x[j + i * di.p], 2.0);
      Theta(l) += (1.0/(sigma*sigma)) * di.x[j + i * di.p] * di.rp[i];
    }
  }
}

// compound-symmetric errors: same as compute_p_theta_cs with scalar prior variance per leaf
                           ...
                           double prior_var_per_leaf)
{
  ...
  std::map<int, std::vector<double>> x_sums;
  std::map<int, double> xx_sums;
  std::map<int, double> xr_sums;
  std::vector<double> r_sums(di.n,0.0);
  ...
  for(suff_stat_it l_it = ss.begin(); l_it != ss.end(); ++l_it){
    leaf_map.insert(std::pair<int, int>(l_it->first, l));
    x_sums.insert(std::pair<int, std::vector<double>>(l_it->first, std::vector<double>(di.n, 0.0)));
    xx_sums.insert(std::pair<int, double>(l_it->first, 0.0));
    xr_sums.insert(std::pair<int, double>(l_it->first, 0.0));
    xs_it = x_sums.find(l_it->first);
    xxs_it = xx_sums.find(l_it->first);
    xrs_it = xr_sums.find(l_it->first);
    ...
    for(int_it it = l_it->second.begin(); it != l_it->second.end(); ++it){
      i = *it;
      subj_id = di.subj_id[i];
      xs_it->second[subj_id] += di.x[j + i*di.p];
      xxs_it->second += pow(di.x[j + i*di.p], 2.0);
      xrs_it->second += di.x[j + i*di.p] * di.rp[i];
      r_sums[subj_id] += di.rp[i];
    }
    ++l;
  }
  ...
  for(suff_stat_it l_it = ss.begin(); l_it != ss.end(); ++l_it){
    l = leaf_map.find(l_it->first)->second;
    P(l,l) += (1.0/(1.0 - rho)) * (1.0/(sigma*sigma)) * xx_sums.find(l_it->first)->second;
    Theta(l) += xr_sums.find(l_it->first)->second;
  }
  ...
  for(int subj_ix = 0; subj_ix < di.n; ++subj_ix){
    for(suff_stat_it l_it = ss.begin(); l_it != ss.end(); ++l_it){
      l = leaf_map.find(l_it->first)->second;
      xs_it = x_sums.find(l_it->first);
      P(l,l) -= (1.0/(sigma*sigma)) * (rho/(1.0 - rho)) *
                (1.0/(1 + rho * ( (double) di.ni[subj_ix] - 1.0))) *
                pow(xs_it->second[subj_ix],2.0);
      Theta(l) -= rho * r_sums[subj_ix]/(1 + rho * ((double) di.ni[subj_ix] - 1.0)) * xs_it->second[subj_ix];

      for(suff_stat_it ll_it = ss.begin(); ll_it != l_it; ++ll_it){
        ll = leaf_map.find(ll_it->first)->second;
        xs_it_ll = x_sums.find(ll_it->first);
        double factor = (1.0/(sigma*sigma)) * (rho/(1.0 - rho)) *
                        (1.0/(1 + rho * ( (double) di.ni[subj_ix] - 1.0)));
        double prod = xs_it->second[subj_ix] * xs_it_ll->second[subj_ix];
        P(l,ll) -= factor * prod;
        P(ll,l) -= factor * prod;
      }
    }
  ...
  Theta *= (1.0/(sigma*sigma)) * (1.0/(1.0 - rho));
  Theta += tree_pi.mu0 / prior_var_per_leaf;
}

// -------------------- log-posteriors for RHS on log-scale --------------------

static inline double safe_log1p_exp2(double x){
  // returns log(1 + exp(2x)) stably
  // for large x, log(1 + e^{2x}) ~ 2x
  if(x > 20.0) return 2.0*x;
  return std::log1p(std::exp(2.0*x));
}
...
double logpost_loglambda_rhs(double log_lambda_j,
                             ...
                             int M)
{
  // v(λ) = (τ^2 λ^2 c2 / (c2 + τ^2 λ^2)) / M
  const double l = log_lambda_j;
  const double t2 = tau * tau;
  const double tl2 = t2 * std::exp(2.0*l); // τ^2 λ^2
  const double num = c2 * tl2;
  const double den = c2 + tl2;

  // guard against degenerate values
  if(!(num > 0.0) || !(den > 0.0)) return -std::numeric_limits<double>::infinity();

  const double logv = std::log(num) - std::log(den) - std::log((double) M);
  const double invv = ((double) M) * (den / num);

  const double loglik = -0.5 * n_mu_j * logv - 0.5 * invv * sumsq_mu_j;

  // half-Cauchy(0,1) prior on λ with log-Jacobian for log λ
  // log p(λ) ∝ -log(1 + λ^2); with λ = e^l, add +l
  const double logprior = - safe_log1p_exp2(l) + l;
...
{
  const double lt = log_tau;
  const double t2 = std::exp(2.0*lt);

  double loglik = 0.0;
  const int J = (int)sumsq_by_j.size();

  for(int j = 0; j < J; ++j){
    const double l2 = lambda_by_j[j] * lambda_by_j[j];
    const double tl2 = t2 * l2;                // τ^2 λ_j^2
    const double num = c2 * tl2;
    const double den = c2 + tl2;
    ...
    loglik += -0.5 * n_by_j[j] * logv - 0.5 * invv * sumsq_by_j[j];
  }

  // half-Cauchy(0, tau0) prior on τ with log-Jacobian for log τ
  // p(τ) ∝ 1 / (1 + (τ/τ0)^2)
  const double logprior = - std::log1p( t2 / (tau0 * tau0) ) + lt;
...
{
  const double lc2 = log_c2;
  const double c2 = std::exp(lc2);
  const double t2 = tau * tau;
  ...
  // IG(a_slab, b_slab) prior on c2 with density ∝ c2^{-(a+1)} exp(-b/c2), plus Jacobian for log c2
  const double logprior = -(a_slab + 1.0) * lc2 - (b_slab / c2) + lc2; // +lc2 is Jacobian

  return loglik + logprior;
}

// --------------------  1-D slice sampler on log scale --------------------
...
{
  // initial log-density at current point
  double logfx0 = logf(x0);
  if(!std::isfinite(logfx0)) return x0; // reject bad starting points gracefully

  // vertical level
  double logy = logfx0 + gen.log_uniform(); // log(U) with U ~ Unif(0,1)

  // create an interval of width w around x0
  double u = gen.uniform(); // in (0,1)
  double L = x0 - u * w;
  double R = L + w;
  ...
  // step out
  int J = (int) std::floor(m * gen.uniform());
  int K = m - 1 - J;

  while(J > 0 && L > lower && logf(L) > logy){
    L = std::max(L - w, lower);
    --J;
  }
  while(K > 0 && R < upper && logf(R) > logy){
    R = std::min(R + w, upper);
    --K;
  }
  ...
  for(int it = 0; it < 1000; ++it){
    double x1 = gen.uniform(L, R);
    double logfx1 = logf(x1);
    if(logfx1 >= logy && std::isfinite(logfx1)) return x1;

    // shrink interval
    if(x1 < x0) L = x1;
    else        R = x1;

    // if interval collapses numerically, return current
    if(R - L < 1e-12) return x0;
  }
