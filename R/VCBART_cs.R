VCBART_cs <- function(Y_train,
  ...
  if(length(X_test) > 1) {
    if(ncol(X_test) != ncol(X_train)) {
      stop(sprintf("X_train has %d columns but X_test has %d columns", ncol(X_train), ncol(X_test)))
    }
  ...
  else if(length(mu0) != ncol(std_X_train)) stop("mu0 needs length 1 + ncol(X_train)")

  if(is.null(tau)) tau <- rep(0.5 / sqrt(M), times = ncol(std_X_train))  
  else if(length(tau) != ncol(std_X_train)) stop("tau needs length 1 + ncol(X_train)")

  if(is.null(nu))     nu     <- 3
  if(is.null(lambda)) lambda <- stats::qchisq(0.1, df = nu) / nu

  # Dirichlet hyperparams for modifier splitting probs
  a_u <- 0.5
  b_u <- 1.0
